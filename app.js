...
function checkAuth() {
  ...
  if (token && userStr) {
    try { AppState.token = token; AppState.user = JSON.parse(userStr); return true; } catch(e) {}
  }
  return false;
}
...
function navigateTo(page) {
  ...
  const titles = {
    dashboard: '🏠 Dashboard', tasks: '✅ Task Manager', planner: '📅 AI Study Planner',
    timer: '⏱️ Focus Timer', calendar: '📆 Calendar', chat: '🤖 AI Assistant',
    flashcards: '🃏 Flashcards', analytics: '📊 Analytics', settings: '⚙️ Settings'
  };
...
function startTimer() {
  ...
  AppState.timer.interval = setInterval(() => {
    ...
    updateTimerDisplay();
    if (AppState.timer.seconds <= 0) timerComplete();
  }, 1000);
  const subject = document.getElementById('timer-subject')?.value;
  api('/sessions', { method: 'POST', body: { type: AppState.timer.mode, plannedMinutes: Math.floor(TIMER_MODES[AppState.timer.mode] / 60), subject: subject || 'General' } })
    .then(data => { AppState.timer.session = data.session; }).catch(() => {});
}
...
function timerComplete() {
  ...
  if (AppState.timer.session) {
    ...
    api(`/sessions/${AppState.timer.session._id}/complete`, { method: 'PUT', body: { actualMinutes: plannedMins, mood: 'good', productivityScore: 8 } }).catch(() => {});
  }
  if (AppState.timer.mode === 'pomodoro') setTimerMode('short-break');
...
async function sendChatMessage(message) {
  ...
  try {
    const data = await api('/ai/chat', { method: 'POST', body: { message, conversationHistory: AppState.chatHistory.slice(-10) } });
...
function addChatMessage(text, role) {
  ...
  if (!container) return;
  const avatarContent = role === 'user' ? (AppState.user?.name?.[0] || 'U').toUpperCase() : '🤖';
  const el = document.createElement('div');
  el.className = `chat-message ${role}`;
  el.innerHTML = `<div class="chat-avatar">${avatarContent}</div><div class="chat-bubble">${formatMarkdown(text)}</div>`;
  container.appendChild(el);
...
async function generateStudyPlan(formData) {
  ...
  try {
    const data = await api('/ai/generate-study-plan', { method: 'POST', body: formData });
    hideLoading();
  ...
  container.innerHTML = `
    <div class="card mb-24">
      ...
      <div class="countdown-grid mb-16">
        <div class="countdown-unit"><div class="countdown-num">${days}</div><div class="countdown-unit-label">Days Left</div></div>
        <div class="countdown-unit"><div class="countdown-num">${plan.weeklyPlan?.length || 1}</div><div class="countdown-unit-label">Weeks</div></div>
      </div>
    </div>
    ...
    <div class="card mb-24">
      ...
      <ul style="list-style:none;display:flex;flex-direction:column;gap:8px;">
        ${plan.learningStrategies.map(s => `<li style="display:flex;align-items:flex-start;gap:8px;"><span style="color:var(--green);margin-top:2px;">✓</span><span class="text-secondary">${escapeHtml(s)}</span></li>`).join('')}
      </ul>
    </div>` : ''}
...
async function generateFlashcards(topic, subject, count) {
  ...
  try {
    const data = await api('/ai/flashcards', { method: 'POST', body: { topic, subject, count: parseInt(count) || 10 } });
    hideLoading();
...
function renderAlarms(alarms) {
  ...
  container.innerHTML = alarms.map(alarm => {
    // Check if this alarm has a custom tune saved
    let hasTune = false;
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('alarm_tune_' + alarm.title)) { hasTune = true; break; }
    }
    return `
    <div class="flex-between" style="padding:10px;background:var(--bg-elevated);border-radius:var(--radius-md);margin-bottom:8px;">
      <div>
        <div class="text-sm font-bold">🔔 ${escapeHtml(alarm.title)} ${hasTune ? '<span style="font-size:11px;color:var(--accent);">🎵 custom tune</span>' : ''}</div>
...
async function setAlarm(title, time, subject, repeat) {
  try {
    await api('/alarms', { method: 'POST', body: { title, time, subject, repeat } });
    await loadAlarms();
...
async function deleteAlarm(id) {
  try {
    // Also clean up any stored tune for this alarm
    const alarm = AppState.alarms.find(a => a._id === id);
    if (alarm) {
      for (let i = localStorage.length - 1; i >= 0; i--) {
        const key = localStorage.key(i);
        if (key && key.startsWith('alarm_tune_' + alarm.title)) localStorage.removeItem(key);
      }
    }
...
function triggerAlarm(alarm) {
  // ── Check for a custom tune stored in localStorage ──
  let customTune = null;
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && key.startsWith('alarm_tune_' + alarm.title)) {
      customTune = localStorage.getItem(key);
      break;
    }
  }

  if (customTune) {
    // Play the user's custom audio file
    try {
      const audio = new Audio(customTune);
      audio.volume = 0.8;
      audio.play().catch(() => playAlarmSound()); // fallback if blocked
...
function calDayClick(dateStr) {
  const dayTasks = AppState.tasks.filter(t => t.dueDate?.startsWith(dateStr));
  if (dayTasks.length) toast(`${dayTasks.length} task(s) due on ${dateStr}`, 'info');
}
...
function renderAnalytics(analytics) {
  ...
  if (chartEl && analytics.dailyData) {
    ...
    chartEl.innerHTML = analytics.dailyData.map(d => {
      ...
      return `<div class="chart-bar-wrap" title="${hrs} hours on ${d.date}"><div class="chart-bar" style="height:${pct}%;"></div><div class="chart-bar-label">${label}</div></div>`;
    }).join('');
  }
  ...
  if (tasksEl && analytics.tasks) {
    ...
    tasksEl.innerHTML = [
      ...
      <div class="mb-8">
        <div class="flex-between mb-4"><span class="text-sm">${item.label}</span><span class="text-sm text-muted">${item.value}</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:${(item.value/total)*100}%;background:${item.color};"></div></div>
      </div>`).join('');
  }
  ...
  if (subjectEl && analytics.subjectBreakdown?.length) {
    ...
    subjectEl.innerHTML = analytics.subjectBreakdown.map(s => `
      <div class="mb-8">
        <div class="flex-between mb-4"><span class="text-sm">${s._id || 'General'}</span><span class="text-sm text-muted">${(s.totalMinutes/60).toFixed(1)}h</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:${(s.totalMinutes/maxMins)*100}%;"></div></div>
      </div>`).join('');
  }
}
...
async function loadDashboard() {
  try {
    ...
    if (motivationEl) {
      api('/ai/motivation').then(data => { motivationEl.textContent = data.message; })
        .catch(() => { motivationEl.textContent = `Welcome back, ${AppState.user?.name}! Ready to crush your goals today? 🚀`; });
    }
...
function updateExamCountdown() {
  ...
  if (diff < 0) { countdownEl.innerHTML = `<p class="text-green">🎉 ${examName} has passed!</p>`; return; }
  ...
  countdownEl.innerHTML = `
    ...
    <div class="countdown-grid">
      <div class="countdown-unit"><div class="countdown-num">${diff}</div><div class="countdown-unit-label">Days</div></div>
...
function loadTheme() {
  const saved = localStorage.getItem('sp_theme') || (AppState.user?.preferences?.theme || 'dark');
...
function initKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'n' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); showModal('task-modal'); }
